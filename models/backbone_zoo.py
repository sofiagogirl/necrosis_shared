...
def backbone_zoo(backbone_name, weights, input_tensor, depth, freeze_backbone, freeze_batch_norm):
    '''
    Configuring a user specified encoder model_G based on the `tensorflow.keras.applications`
    
    Input
    ----------
        backbone_name: the bakcbone model_G name. Expected as one of the `tensorflow.keras.applications` class.
    ...
    Output
    ----------
        model_G: a keras backbone model_G.

...
def interpn(vol, loc, interp_method='linear'):
    ...
    # print('this is nb_dims')
    # print(nb_dims)
    # print('this is vol.shape')
    # print(vol.shape[:-1])
...
def batch_affine_to_shift(affine_matrix, volshape, shift_center=True, indexing='ij', batch_size=16):
    ...
    if len(affine_matrix.shape) == 2:
        # if len(affine_matrix[0]) != (nb_dims * (nb_dims + 1)) :
        #     raise ValueError('transform is supposed a vector of len ndims * (ndims + 1).'
        #                      'Got len %d' % len(affine_matrix[0]))
    ...
    for b in range(batch_size):
        ...
        # compute locations
        loc_matrix = tf.matmul(affine_matrix[b], mesh_matrix)  # N+1 x nb_voxels
        loc_matrix = tf.transpose(loc_matrix[:nb_dims, :])  # nb_voxels x N
        loc = tf.reshape(loc_matrix, list(volshape) + [nb_dims])  # *volshape x N
        # loc = [loc[..., f] for f in range(nb_dims)]  # N-long list, each entry of shape volshape
...
def transform(vol, loc_shift, interp_method='linear', indexing='ij'):
    ...
    nb_dims = len(volshape)
    # print('this is transform function nb_dims')
    # print(nb_dims)
    # print('this is transform function volshape')
    # print(vol.shape)
    ...
    #print('this is transform function locshape')
    #print(loc.shape)
    # test single

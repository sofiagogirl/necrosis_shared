...
from tensorflow.keras.layers import Conv2D, DepthwiseConv2D, Lambda, Dense, Conv3D
    ...
    * The defaut: `kernel_size=3`, is suitable for `pool_size=2`.
    ...
    if unpool:
        ...
        X = Conv2DTranspose(channel, kernel_size, strides=(pool_size, pool_size), 
                            padding='same', name='{}_trans_conv'.format(name), kernel_initializer='glorot_normal')(X)
...
def attention_gate(X, g, channel,  
    ...
    theta_att = Conv2D(channel, 1, use_bias=True, name='{}_theta_x'.format(name), kernel_initializer='glorot_normal')(X)
    
    # mapping the gate tensor
    phi_g = Conv2D(channel, 1, use_bias=True, name='{}_phi_g'.format(name), kernel_initializer='glorot_normal')(g)
    ...
    psi_f = Conv2D(1, 1, use_bias=True, name='{}_psi_f'.format(name), kernel_initializer='glorot_normal')(f)
    # ------------------------------ #
...
def CONV_stack(X, channel, kernel_size=3, stack_num=2, 
    ...
    for i in range(stack_num):
        ...
        X = Conv2D(channel, kernel_size, padding='same', use_bias=bias_flag, 
                   dilation_rate=dilation_rate, name='{}_{}'.format(name, i), kernel_initializer='glorot_normal')(X)
...
def CONV_stack_3D_to_2D(X, channel, kernel_size=3, z_kernel_size=3, stack_num=2, dilation_rate=1, activation='ReLU',
                        batch_norm=False, name='conv_stack'):
    '''
    Stacked 3D convolutional layers with z valid padding to form 2D final output:
    (Convolutional layer --> batch normalization --> Activation)*stack_num

    CONV_stack_3D_to_2D(X, channel, kernel_size=3, stack_num=2, dilation_rate=1, activation='ReLU',
                        batch_norm=False, name='conv_stack')
    ...
    for i in range(stack_num):
        ...
        # linear convolution
        X = conv3D_z_valid(X, channel, kernel_size, z_kernel_size, padding='same', use_bias=bias_flag,
    ...
    X = squeeze(X, axis=3)  # squeeze the depth channel
...
def conv3D_z_valid(X, channel, kernel_size, z_kernel_size, padding, use_bias, dilation_rate, name):
    ...
    # 3d convolution with valid padding only along z (depth dimension)
    X = Conv3D(channel, kernel_size=[kernel_size, kernel_size, z_kernel_size], padding=padding, use_bias=use_bias,
               dilation_rate=dilation_rate, name=name, kernel_initializer='glorot_normal')(X)
    X = X[:, :, :, z_kernel_size//2:-(z_kernel_size//2), :]
...
def CONV_output(X, n_labels, kernel_size=1, activation='Softmax', name='conv_output'):
    ...
    X = Conv2D(n_labels, kernel_size, padding='same', use_bias=True, name=name, kernel_initializer='glorot_normal')(X)
...
def dense_layer(X, units, activation='LeakyReLU', name='dense_layer'):
    X = Dense(units, activation=activation, use_bias=True,

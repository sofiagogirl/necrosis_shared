...
def init_parameters():
    ...
    # ======================= paths ==========================
    tc.model_path = 'INSERT_MODEL_SAVE_PATH_HERE'
    tc.prev_checkpoint_path = None
    tc.save_every_epoch = True

    # pretrained checkpoints to warm start from (set to None if not used)
    ...
    # ======================= data paths =====================
    tc.image_path_1 = 'INSERT_TRAIN_DATA_PATH_HERE/*.npy'
    vc.image_path_1 = ''
    tc.image_path_2 = ''
    vc.image_path_2 = ''
    ...
    tc.lamda = 60.0  # adversarial loss weight
    ...
    tc.n_shuffle_epoch, vc.n_shuffle_epoch = 500, 5000
    ...
    tc.case_filtering_cur_mean = 0.3757
    tc.case_filtering_cur_stdev = 0.0654
...
def run_validation(vc, cur_iter, iterator_valid_bl, G_test_step, D_test_step, R_test_step, wtr, use_tqdm=True):
    valid_G_total_loss_list = []
    valid_G_l1_loss_list = []
    valid_G_ssim_list = []
    valid_G_psnr_list = []
    valid_G_ncc_list = []
    valid_D_real_loss_list = []
    valid_D_fake_loss_list = []
    valid_R_total_loss_list = []
    ...
    for i in valid_idx_iterator:
        ...
        valid_G_total_loss, valid_G_dis_loss, valid_G_l1_loss, valid_G_ssim, valid_G_psnr, valid_G_ncc = G_test_step(valid_x, valid_y, 3)
        ...
        # save sample outputs for the first 5 batches
        if i < 5:
            for j in range(vc.batch_size):
                base_path = tc.model_path + f'/output/iter={cur_iter}_sample={i * vc.batch_size + j}'
                if tc.label_channels == 1:
                    plt.imsave(base_path + '_outputG.jpg', valid_G_output[j, :, :, 0], cmap='gray')
                    plt.imsave(base_path + '_outputR.jpg', valid_R_output[0][j, :, :, 0], cmap='gray')
                    plt.imsave(base_path + '_target.jpg', valid_y[j, :, :, 0], cmap='gray')
                else:
                    plt.imsave(base_path + '_outputG.jpg', np.clip(valid_G_output[j, :, :, :].numpy(), 0, 1))
                    plt.imsave(base_path + '_outputR.jpg', np.clip(valid_R_output[0][j, :, :, :].numpy(), 0, 1))
                    plt.imsave(base_path + '_target.jpg', valid_y[j, :, :, :].numpy())
                    plt.imsave(base_path + '_input0.jpg', -valid_x[j, :, :, 0].numpy(), cmap='gray')
    ...
    valid_G_total_loss_mean = np.mean(valid_G_total_loss_list)
    valid_G_l1_loss_mean = np.mean(valid_G_l1_loss_list)
    valid_G_ssim_mean = np.mean(valid_G_ssim_list)
    valid_G_psnr_mean = np.mean(valid_G_psnr_list)
    valid_G_ncc_mean = np.mean(valid_G_ncc_list)
    valid_G_ncc_std = np.std(valid_G_ncc_list)
    valid_D_real_loss_mean = np.mean(valid_D_real_loss_list)
    valid_D_fake_loss_mean = np.mean(valid_D_fake_loss_list)
    valid_R_total_loss_mean = np.mean(valid_R_total_loss_list)
...
if __name__ == '__main__':
    ...
    print('Train image examples:', train_images[:5])
    print('Number of valid images:', len(valid_images))
    print('Valid image examples:', valid_images[:5])

    train_bl = ImageTransformationBatchLoader(
        train_images, tc, tc.num_slices, is_testing=False,
        n_parallel_calls=tc.n_threads, q_limit=tc.q_limit, n_epoch=tc.n_shuffle_epoch)
    valid_bl = ImageTransformationBatchLoader(
        valid_images, vc, vc.num_slices, is_testing=True,
    ...
    # ======================= models =========================
    model_G = att_unet_2d(
        (tc.image_size, tc.image_size, tc.num_slices), n_labels=tc.label_channels,
    ...
    model_D = discriminator_2d(
        (tc.image_size, tc.image_size, tc.label_channels),
        filter_num=[tc.n_channels, tc.n_channels * 2, tc.n_channels * 4, tc.n_channels * 8, tc.n_channels * 16],
        stack_num_down=1, activation='LeakyReLU',
        batch_norm=False, pool=False, name='model_D')

    model_R = aligner_unet_cvpr2018_vJX(
        [tc.image_size, tc.image_size], tc.nf_enc, tc.nf_dec,
    ...
    # load warm start checkpoints if provided
    ...
    if tc.prev_checkpoint_path:
        ...
        print("Loaded latest checkpoints from", tc.prev_checkpoint_path)
    ...
    def G_test_step(input_image, target, epoch):
        ...
        G_outputs_clipped_split = split_tensor(tf.clip_by_value(G_outputs, 0, 1),
                                               tc.case_filtering_x_subdivision, tc.case_filtering_y_subdivision)
        target_clipped_split = split_tensor(tf.clip_by_value(target, 0, 1),
                                            tc.case_filtering_x_subdivision, tc.case_filtering_y_subdivision)
        ...
        G_ncc = tf.reduce_mean(NCC(win=20, eps=1e-3).ncc(target_clipped_split, G_outputs_clipped_split))
    ...
    for epoch in range(epoch_begin, tc.N_epoch):
        ...
        if epoch > 0 or (tc.G_warmstart_checkpoint is None and tc.prev_checkpoint_path is None):
            ...
            if tc.case_filtering and epoch >= tc.case_filtering_starting_epoch:
                print(f"Case filtering ON for epoch {epoch} | threshold: "
                      f"{tc.case_filtering_cur_mean - tc.case_filtering_nsigma * tc.case_filtering_cur_stdev:.4f} "
                      f"| subdivision: {tc.case_filtering_x_subdivision}x{tc.case_filtering_y_subdivision}")
            else:
                print(f"Case filtering OFF for epoch {epoch}")
            ...
            for i in tqdm(range(num_checkpoint_D)):
                ...
                for j in range(NumGen):
                    ...
                    train_G_total_loss, train_G_dis_loss, train_G_l1_loss, train_G_output, train_R_output = G_train_step(train_x, train_y, epoch)
                    train_G_total_loss_list.append(train_G_total_loss)
                    train_G_l1_loss_list.append(train_G_l1_loss)
                    wtr.check_stop()

                # save sample outputs every 1000 steps
                if i % 1000 == 0:
                    base_path = tc.model_path + f'/output/train_epoch={epoch}_sample={i}'
                    plt.imsave(base_path + '_outputG.jpg', np.clip(train_G_output[0, :, :, :].numpy(), 0, 1))
                    plt.imsave(base_path + '_outputR.jpg', np.clip(train_R_output[0, :, :, :].numpy(), 0, 1))
                    plt.imsave(base_path + '_target.jpg', train_y[0, :, :, :].numpy())
                    plt.imsave(base_path + '_input0.jpg', -train_x[0, :, :, 0].numpy(), cmap='gray')
                ...
                if iter_D_count % tc.valid_steps == 0 and i != num_checkpoint_D - 1:
                    (valid_G_total_loss_mean, valid_G_l1_loss_mean, valid_G_ssim_mean, valid_G_psnr_mean,
                     ...
                     valid_R_total_loss_mean) = run_validation(vc, iter_D_count, iterator_valid_bl,
                                                               G_test_step, D_test_step, R_test_step, wtr, use_tqdm=False)

                    train_G_total_loss_mean = np.mean(train_G_total_loss_list)
                    train_G_l1_loss_mean = np.mean(train_G_l1_loss_list)
                    train_R_total_loss_mean = np.mean(train_R_total_loss_list)
                    train_G_total_loss_list, train_G_l1_loss_list, train_R_total_loss_list = [], [], []

                    if tc.case_filtering and tc.case_filtering_recalc_every_eval:
                        tc.case_filtering_cur_mean = valid_G_ncc_mean
                        tc.case_filtering_cur_stdev = valid_G_ncc_std
                        print(f"Updated filtering mean to {valid_G_ncc_mean:.4f} and stdev to {valid_G_ncc_std:.4f}")

                    text = ["round", "iter_D_count", "train_G_total_loss_mean", "train_G_l1_loss_mean",
                            "valid_G_total_loss_mean", "valid_G_l1_loss_mean", "valid_G_ssim_mean",
                            "valid_G_psnr_mean", "valid_G_ncc_mean", "valid_G_ncc_std",
                    ...
                    if min_loss - valid_G_l1_loss_mean > tc.min_del or valid_G_psnr_mean > max_psnr or tc.save_every_epoch:
                        ...
                        if min_loss - valid_G_l1_loss_mean > tc.min_del:
                            print(f'Validation loss improved from {min_loss:.4f} to {valid_G_l1_loss_mean:.4f}')
                            min_loss = valid_G_l1_loss_mean

                        if valid_G_psnr_mean > max_psnr:
                            print(f'Validation PSNR improved from {max_psnr:.4f} to {valid_G_psnr_mean:.4f}')
                            max_psnr = valid_G_psnr_mean

                    model_G.save_weights(tc.model_path + '/model_G_latest.h5')
                    model_D.save_weights(tc.model_path + '/model_D_latest.h5')
                    model_R.save_weights(tc.model_path + '/model_R_latest.h5')
                    np.save(tc.model_path + '/optimizer_G_latest.npy', G_optimizer.get_weights())
                    np.save(tc.model_path + '/optimizer_D_latest.npy', D_optimizer.get_weights())
                    np.save(tc.model_path + '/optimizer_R_latest.npy', R_optimizer.get_weights())
        ...
        (valid_G_total_loss_mean, valid_G_l1_loss_mean, valid_G_ssim_mean, valid_G_psnr_mean,
         valid_G_ncc_mean, valid_G_ncc_std, valid_D_real_loss_mean, valid_D_fake_loss_mean,
         valid_R_total_loss_mean) = run_validation(vc, iter_D_count, iterator_valid_bl,
                                                   G_test_step, D_test_step, R_test_step, wtr)

...
def init_parameters():
    ...
    # tc.G_warmstart_checkpoint = "experiments_MT/stage2_20220328_G&RSeperateTrain_initIter=6000/model_G_round=32.h5"
    tc.D_warmstart_checkpoint = None
    # tc.D_warmstart_checkpoint = "experiments_MT/stage2_20220328_G&RSeperateTrain_initIter=6000/model_D_round=32.h5"
    tc.R_warmstart_checkpoint = None
    # tc.R_warmstart_checkpoint = "experiments_MT/stage2_20220328_G&RSeperateTrain_initIter=6000/model_R_round=32.h5"
    ...
    # tc.image_dir = 'F:/HER2/RoundF/target/'
        ...
        if i < 5:
            for j in range(vc.batch_size):
                if tc.label_channels == 1:
                    ...
                    plt.imsave(tc.model_path + f'/output/iter={cur_iter}_sample={i * vc.batch_size + j}_input1.jpg',
                               -valid_x[j, :, :, 1].numpy(), cmap='gray')
                    plt.imsave(tc.model_path + f'/output/iter={cur_iter}_sample={i * vc.batch_size + j}_input2.jpg',
                               -valid_x[j, :, :, 2].numpy(), cmap='gray')
                    plt.imsave(tc.model_path + f'/output/iter={cur_iter}_sample={i * vc.batch_size + j}_input3.jpg',
                               -valid_x[j, :, :, 3].numpy(), cmap='gray')
...
if __name__ == '__main__':
    ...
    # saving a file with the configuration
    config_path = tc.model_path + f'/config.txt'
    config_content = ""
    for attr, value in tc.items():
        if not callable(value):
            config_content = config_content + attr + ": " + str(value) + "\n"
    tf.io.write_file(config_path, config_content)
    ...
    for epoch in range(epoch_begin, tc.N_epoch):
        ...
        # drop = 0.1
        # epoch_to_drop = 8
        # new_R_learning_rate = init_R_learning_rate * (drop ** np.floor(epoch/epoch_to_drop))
        # R_optimizer.lr.assign(new_R_learning_rate)
        ...
        if epoch > 0 or (tc.G_warmstart_checkpoint is None and tc.prev_checkpoint_path is None):
            ...
            for i in tqdm(range(num_checkpoint_D)):
                ...
                # getting rid of mid-round validation
                '''
